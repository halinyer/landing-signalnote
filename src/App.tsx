...
const GlobalBackground = ({ isDark }: { isDark: boolean }) => (
  <div className={`fixed inset-0 pointer-events-none -z-50 overflow-hidden transition-colors duration-1000 ${isDark ? 'bg-[#0a0a0a]' : 'bg-[#FAFAFA]'}`}>
    ...
    <div className={`absolute top-[-10%] left-[-10%] w-[50vw] h-[50vw] max-w-[600px] max-h-[600px] bg-[#2FC7EA] rounded-full blur-[120px] ${isDark ? 'opacity-10' : 'opacity-15'} animate-pulse`} style={{ animationDuration: '8s' }} />
    <div className={`absolute top-[20%] right-[-10%] w-[40vw] h-[40vw] max-w-[500px] max-h-[500px] bg-[#EF007E] rounded-full blur-[120px] ${isDark ? 'opacity-5' : 'opacity-10'} animate-pulse`} style={{ animationDuration: '12s' }} />
  </div>
);
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-700 ${isDarkMode ? 'bg-[#0a0a0a] text-neutral-200' : 'text-neutral-900'} selection:bg-[#2FC7EA] selection:text-white`}>
      ...
      <nav className={`fixed top-0 w-full z-50 ${isDarkMode ? 'bg-[#0a0a0a]/70 border-white/10' : 'bg-[#FAFAFA]/70 border-white/20'} backdrop-blur-xl border-b transition-colors duration-500`}>
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-2 h-2 rounded-full ${isDarkMode ? 'bg-white' : 'bg-neutral-900'} animate-pulse`}></div>
            <span className={`font-semibold tracking-tight ${isDarkMode ? 'text-white' : 'text-neutral-900'}`}>signalNote</span>
          </div>
          <div className="hidden md:flex gap-6 items-center text-xs font-semibold uppercase tracking-widest text-neutral-500">
            ...
            <button onClick={() => setIsDarkMode(!isDarkMode)} className={`p-2 rounded-full ${isDarkMode ? 'hover:bg-white/10' : 'hover:bg-neutral-900/5'} transition-colors flex items-center justify-center`}>
              {isDarkMode ? <Sun size={16} className="text-neutral-400 hover:text-white" /> : <Moon size={16} className="text-neutral-500 hover:text-neutral-900" />}
            </button>
          </div>
        </div>
      </nav>
      ...
      <header className="relative min-h-screen flex flex-col justify-center items-center text-center px-6 pt-20">
        ...
        >
          <div className={`inline-flex items-center gap-2 mb-8 ${isDarkMode ? 'bg-white/5 border-white/10 text-neutral-300' : 'bg-white/60 border-neutral-200/60 text-neutral-600'} backdrop-blur-md px-4 py-2 rounded-full text-[10px] sm:text-xs font-medium tracking-widest uppercase shadow-sm transition-colors duration-500`}>
            <span className="w-1.5 h-1.5 rounded-full bg-[#2FC7EA]"></span>
            Agencia de Infraestructura Visual
          </div>
          
          <h1 className={`text-6xl sm:text-7xl md:text-8xl lg:text-[7.5rem] font-semibold tracking-tighter leading-[0.95] ${isDarkMode ? 'text-white' : 'text-neutral-900'} mb-8 transition-colors duration-500`}>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          >
            ...
            <motion.div 
              ...
              className={`absolute z-10 w-36 h-36 ${isDarkMode ? 'bg-neutral-950 border-white/10' : 'bg-white border-neutral-100'} shadow-[0_0_40px_rgba(47,199,234,0.2)] rounded-full flex flex-col items-center justify-center group-hover:scale-105 transition-all duration-700 ease-out`}
            >
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
            ...
            >
              ...
              <p className="text-sm text-neutral-500 font-light mb-6">Mantenimiento de presencia digital y diseño base.</p>
              <div className="mb-8">
            ...
            >
              ...
              <p className="text-sm text-neutral-500 font-light mb-6">Marketing integral y producción visual de alto nivel.</p>
              <div className="mb-8">
